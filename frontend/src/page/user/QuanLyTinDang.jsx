...
const QuanLyTinDang = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `https://xdudweb-php.onrender.com/api/xoa-tin-dang/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleEdit = (post) => {
    // Map đầy đủ dữ liệu bao gồm relations từ backend
    const editData = {
      id: post.id,
      tieu_de: post.tieu_de,
      mo_ta: post.mo_ta,
      gia_thue: post.gia_thue || post.gia || "",
      dien_tich: post.dien_tich || "",
      ma_loai_phong: post.loai_phong?.id || post.ma_loai_phong || "",
      trang_thai: post.trang_thai || "hoat_dong",
      vi_tri: post.vi_tri
        ? {
            tinh_thanh_pho: post.vi_tri.tinh_thanh_pho || "",
            quan_huyen: post.vi_tri.quan_huyen || "",
            phuong_xa: post.vi_tri.phuong_xa || "",
            ten_duong: post.vi_tri.ten_duong || "",
            dia_chi_chi_tiet: post.vi_tri.dia_chi_chi_tiet || "",
          }
        : {},
      hinh_anh: post.hinh_anh
        ? post.hinh_anh.map((img) => ({ duong_dan_anh: img.duong_dan_anh }))
        : [],
      tien_ich: post.tien_ich || [],
    };
    // Truyền editData thông qua state để DangTin.jsx populate form
  ...
  return (
    <div className="container py-4 min-vh-100">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button
          className="btn btn-primary text-white fw-bold shadow-sm px-4 py-2"
                ...
                {isLoading ? (
                  ...
                  filteredPosts.map((post) => (
                    <tr key={post.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${post.trang_thai === "hoat_dong" ? "bg-success" : "bg-secondary"}`}
                        >
                          {post.trang_thai === "hoat_dong"
                            ? "Đang hoạt động"
                            : "Đã ẩn/Chờ duyệt"}
                        </span>
                      </td>
