...
import { MapPin, Navigation, Loader2, ArrowRight, Filter, Search, X } from "lucide-react";
...
const Home = () => {
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/tim-kiem-tin-dang?${params.toString()}`);
  ...
  const handleReset = () => {
    const resetValues = {
      tu_khoa: "", gia_tu: "", gia_den: "", dien_tich_tu: "", dien_tich_den: "", ma_loai_phong: "", tinh_thanh_pho: "", quan_huyen: ""
    };
    setFilters(resetValues);
    setSearchParams({}); // Xóa param trên URL
    
    // Gọi lại API ngay lập tức với bộ filter trống
    axios.get(`${API_BASE_URL}/api/tim-kiem-tin-dang`)
      ...
      <main className="container pt-4">
                ...
                <div className="row g-3">
                  ...
                  <div className="col-12 col-md-4">
                    ...
                    <div className="input-group">
                      ...
                      <input type="text" name="tu_khoa" className="form-control border-start-0 bg-light" placeholder="Tên đường, phường, mô tả..." value={filters.tu_khoa} onChange={handleInputChange} />
                    </div>
                  </div>

                  {/* Loại phòng */}
                  <div className="col-12 col-md-4">
                    <label className="form-label text-muted small fw-bold">Loại phòng</label>
                    <select name="ma_loai_phong" className="form-select bg-light" value={filters.ma_loai_phong} onChange={handleInputChange}>
                  ...
                  <div className="col-12 col-md-4">
                    ...
                    <input type="text" name="quan_huyen" className="form-control bg-light" placeholder="VD: Quận 1, Tân Bình..." value={filters.quan_huyen} onChange={handleInputChange} />
                  </div>

                  {/* Mức giá */}
                  <div className="col-12 col-md-6">
                    <label className="form-label text-muted small fw-bold">Mức giá (VNĐ)</label>
                    <div className="d-flex align-items-center gap-2">
                      <input type="number" name="gia_tu" className="form-control bg-light" placeholder="Từ..." value={filters.gia_tu} onChange={handleInputChange} />
                      <span className="text-muted">-</span>
                      <input type="number" name="gia_den" className="form-control bg-light" placeholder="Đến..." value={filters.gia_den} onChange={handleInputChange} />
                    </div>
                  </div>

                  {/* Diện tích */}
                  <div className="col-12 col-md-6">
                    <label className="form-label text-muted small fw-bold">Diện tích (m²)</label>
                    <div className="d-flex align-items-center gap-2">
                      <input type="number" name="dien_tich_tu" className="form-control bg-light" placeholder="Từ..." value={filters.dien_tich_tu} onChange={handleInputChange} />
                      <span className="text-muted">-</span>
                      <input type="number" name="dien_tich_den" className="form-control bg-light" placeholder="Đến..." value={filters.dien_tich_den} onChange={handleInputChange} />
                    </div>
                  </div>

                  {/* Nút hành động */}
                  <div className="col-12 mt-4 d-flex justify-content-end gap-2">
                    <button type="button" className="btn btn-light border text-secondary px-4 fw-medium" onClick={handleReset}>
                      Xóa lọc
                    </button>
                    <button type="submit" className="btn btn-primary px-4 fw-medium d-flex align-items-center gap-2">
        ...
        {loading ? (
          <div className="d-flex justify-content-center align-items-center flex-column py-5 my-5">
            ...
            <p className="text-muted fw-medium">Đang tìm kiếm dữ liệu phù hợp...</p>
          </div>
        ) : rooms.length === 0 ? (
          <div className="text-center py-5 my-5">
            <img src="https://cdn-icons-png.flaticon.com/512/7486/7486831.png" alt="No data" width="120" className="opacity-50 mb-3" />
            <h5 className="text-muted fw-bold">Không tìm thấy tin đăng nào!</h5>
            <p className="text-secondary small">Vui lòng thử lại với các tiêu chí tìm kiếm khác.</p>
          </div>
        ) : (
                ...
                <Link to={`/chi-tiet-tin-dang/${room.id}`} className="text-decoration-none">
                  <article className="card h-100 shadow-sm border-0 rounded-4 overflow-hidden custom-card-hover">
                    ...
                    <div className="card-body d-flex flex-column p-4">
                      <h5 className="card-title fw-bold text-dark text-truncate mb-2" title={room.tieu_de}>
                        {room.tieu_de}
                      </h5>
                      <div className="d-flex align-items-center text-secondary small mb-3">
                        ...
                        <span className="text-truncate">
                          {room.vi_tri?.ten_duong || room.vi_tri?.ten_vi_tri || "Chưa cập nhật địa điểm"}
                        </span>
                      </div>
