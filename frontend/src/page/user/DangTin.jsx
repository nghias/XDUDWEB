...
import {
  ...
  Home,
  ArrowLeft,
} from "lucide-react"; // Thêm ArrowLeft ở đây
...
const DangTin = () => {
  ...
  useEffect(() => {
    if (formData.tinh_thanh_pho) {
      const code = provinces.find(
        (p) => p.name === formData.tinh_thanh_pho,
      )?.code;
      if (code)
        axios
  ...
  useEffect(() => {
    if (editData) {
      ...
      if (editData.tien_ich)
        setSelectedTienIch(editData.tien_ich.map((i) => i.id));
      // Hiển thị ảnh cũ từ database (link Cloudinary)
      if (editData.hinh_anh)
        setPreviews(editData.hinh_anh.map((img) => img.duong_dan_anh));
    }
  }, [editData]);
  ...
  const handleFileChange = (e) => {
    ...
    // Cộng dồn file để gửi lên Server
...
const removeImage = (indexToRemove) => {
  // Xóa preview (để giao diện biến mất)
  setPreviews(prev => prev.filter((_, index) => index !== indexToRemove));
  
  // Xóa file trong mảng gửi đi (để Backend không nhận ảnh đó)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editData
        ? `${API_URL}/api/cap-nhat-tin-dang/${editData.id}`
        : `${API_URL}/api/tao-tin-dang`;
      ...
      alert(
        "Lỗi: " + (error.response?.data?.message || "Dữ liệu không hợp lệ"),
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="container py-5 bg-light min-vh-100">
      <div className="row justify-content-center">
        <div className="col-lg-9">
            ...
            <div className="bg-primary p-4 text-white d-flex align-items-center gap-3">
              ...
              <div>
                <h3 className="fw-bold mb-0">
                  {editData ? "Chỉnh sửa tin đăng" : "Đăng tin cho thuê mới"}
                </h3>
                <p className="mb-0 opacity-75">
                  Thông tin càng chi tiết, khách thuê càng dễ tìm thấy bạn
                </p>
              </div>
            </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                  <label className="form-label fw-medium">
                    Tiêu đề tin đăng <span className="text-danger">*</span>
                  </label>
                  <input
                    type="text"
                    className="form-control form-control-lg"
                    placeholder="VD: Phòng trọ giá rẻ gần STU, đầy đủ tiện nghi..."
                  ...
                  <div className="col-md-4">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          ma_loai_phong: e.target.value,
                        })
                      }
                    >
                      {danhMucLoaiPhong.map((l) => (
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    className="form-control"
                    rows="5"
                    placeholder="Mô tả về giờ giấc, điện nước, nội thất..."
                ...
                <div className="row g-3 mb-3">
                  ...
                  <div className="col-md-4">
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, quan_huyen: e.target.value })
                      }
                      disabled={!formData.tinh_thanh_pho}
                      required
                    >
                  ...
                  <div className="col-md-4">
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, phuong_xa: e.target.value })
                      }
                      disabled={!formData.quan_huyen}
                      required
                    >
                ...
                <div className="row g-3 mb-5">
                  ...
                  <div className="col-md-7">
                    <input
                      type="text"
                      className="form-control"
                      placeholder="Số nhà, hẻm chi tiết..."
                ...
                <div className="mb-4">
                  <label
                    className="w-100 p-5 border border-2 border-dashed rounded-4 d-flex flex-column align-items-center justify-content-center bg-light cursor-pointer"
                    style={{ cursor: "pointer" }}
                  >
                    <input
                      type="file"
                  ...
                  {previews.length > 0 && (
                    <div className="d-flex gap-3 mt-4 overflow-auto pb-2">
                      {previews.map((url, i) => (
                        <div
                          key={i}
                          className="position-relative flex-shrink-0 shadow-sm rounded overflow-hidden"
          ...
          className="position-absolute top-0 end-0 bg-danger text-white border-0 d-flex align-items-center justify-content-center shadow-sm"
          style={{ 
            width: 22, 
            height: 22, 
            borderRadius: '0 0 0 8px', // Bo góc dưới bên trái cho đẹp
            cursor: 'pointer',
            zIndex: 10
          }}
        >
          &times; 
                          ...
                          <div
                            className="position-absolute top-0 start-0 bg-dark bg-opacity-50 text-white px-2 py-1"
                            style={{ fontSize: 10 }}
                          >
                            {i === 0 ? "Ảnh bìa" : `Ảnh ${i + 1}`}
                          </div>
                ...
                <div className="pt-4 border-top mt-5">
                  <button
                    type="submit"
                    disabled={loading}
                    className="btn btn-primary w-100 py-3 fw-bold rounded-pill shadow d-flex align-items-center justify-content-center gap-2 fs-5"
                  >
                    {loading ? (
                      <>
                      ...
                      <>
                        <Send size={20} />{" "}
                        {editData ? "Xác nhận cập nhật" : "Đăng tin ngay"}
                      </>
                    )}
                  </button>
                  <p className="text-center text-muted small mt-3">
                    Bằng việc nhấn Đăng tin, bạn đồng ý với các Điều khoản dịch
                    vụ của chúng tôi
                  </p>
                </div>
              </form>
